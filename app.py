...
# ------------------------------------------------------------
# 승부차기 미니게임 (방별 랭킹/상태 분리: botGroupKey 지원)
# ------------------------------------------------------------
import random, threading
from flask import request, jsonify

# ========================= 전역 저장소 (방 단위) =========================
# game state: 방+유저 단위로 key
PENALTY_GAMES = {}   # { "room:uid": {"shots":[True/False...], "max":5} }
PG_LOCK = threading.Lock()

# career/랭킹: 방 단위로 분리
CAREER = {}          # { room: { uid: {"goals":int, "shots":int} } }
C_LOCK = threading.Lock()

# (선택) 닉네임 캐싱 - 멘션 미지원 환경 fallback 용
NAMEBOOK = {}        # { uid: "nickname" }
N_LOCK = threading.Lock()
...
# ========================= 기본 식별자 헬퍼 =========================
def _uid(body: dict) -> str:
    """카카오 user.id (botUserKey)를 최우선으로 사용"""
    user = ((body.get("userRequest") or {}).get("user") or {})
    uid = (user.get("id") or "").strip()
    return uid or "unknown"
...
def _room(body: dict) -> str:
    """
    채팅방 식별자 추출:
    1) botGroupKey (버튼/파라미터로 들어오는 값)
    2) conversation.id (카카오 대화 세션 ID)
    3) roomId (환경별 폴백)
    4) 개인DM: dm:{user.id}
    """
    ...
    # 1) action.params / userRequest.params 내 botGroupKey 우선
    for container in (action.get("params") or {}, ur.get("params") or {}):
        bg = container.get("botGroupKey")
        if bg:
            return str(bg)

    # 2) 카카오 컨버세이션 ID
    conv = ur.get("conversation") or {}
    rid = conv.get("id")
    if rid:
        return str(rid)
...
def _get_name(uid: str, fallback_short: bool = True) -> str:
    ...
    if nm:
        return nm
    return (uid[:6] if uid else "unknown") if fallback_short else uid


# ========================= 커리어/랭킹 유틸 (방 스코프) =========================
def _career_add(room: str, uid: str, goals: int, shots: int):
    if shots <= 0:
        return
    with C_LOCK:
        roombox = CAREER.setdefault(room, {})
        st = roombox.setdefault(uid, {"goals": 0, "shots": 0})
        st["goals"] += goals
        st["shots"] += shots

def _career_rate(room: str, uid: str):
    with C_LOCK:
        st = (CAREER.get(room) or {}).get(uid)
        if not st or st["shots"] <= 0:
            return None
        return st["goals"] / st["shots"]

def _leaders(room: str):
    """해당 방의 리더보드 [(uid, rate, goals, shots), ...]"""
    with C_LOCK:
        roombox = CAREER.get(room, {})
        items = []
        for k, v in roombox.items():
            shots = v.get("shots", 0)
            goals = v.get("goals", 0)
            if shots > 0:
                rate = goals / shots
...
def _format_leaderboard_and_mentions(room: str, uid: str, limit: int = 10):
    items = _leaders(room)
    if not items:
        text = "아직 기록이 없습니다.\n승부차기를 먼저 플레이해 주세요!"
        mentions = {"user1": {"type": "botUserKey", "id": uid}}
...
# ========================= 게임 상태 유틸 (방+유저 스코프) =========================
def _start(room: str, uid: str):
    key = f"{room}:{uid}"
    with PG_LOCK:
        PENALTY_GAMES[key] = {"shots": [], "max": 5}

def _state(room: str, uid: str):
    key = f"{room}:{uid}"
    with PG_LOCK:
        return PENALTY_GAMES.get(key)

def _reset(room: str, uid: str):
    key = f"{room}:{uid}"
    with PG_LOCK:
        if key in PENALTY_GAMES:
            del PENALTY_GAMES[key]

def _record(room: str, uid: str, success: bool):
    key = f"{room}:{uid}"
    with PG_LOCK:
        st = PENALTY_GAMES.setdefault(key, {"shots": [], "max": 5})
        st["shots"].append(success)
        done = len(st["shots"]) >= st["max"]
        if done:
            final = st["shots"][:]
...
def kakao_penalty():
    try:
        # -------------- 멘트/연출 유틸 --------------
        def _pick(arr): return random.choice(arr) if arr else ""
        GOAL_BASE = ["🔥 절정의 컨디션!","💥 강슛이네요!","🥳 완벽한 코스!","😎 침착했다!","🎯 정확도 미쳤다!","🚀 골망이 찢어지겠어!"]
        MISS_BASE = ["😰 긴장했나 봐요!","🧤 골키퍼 선방!","🙈 아깝다, 포스트!","😵 살짝 빗나갔어요.","😬 다음엔 더 과감하게!","🌪️ 페인트에 걸렸나?"]
        ...
        body = request.get_json(silent=True) or {}
        uid  = _uid(body)
        room = _room(body)    # ✅ 방 식별자
        uname = _uname(body)
        _save_name(uid, uname)

        uter = (body.get("userRequest") or {}).get("utterance") or ""
        st = _state(room, uid)
        GM_id = ((body.get("userRequest") or {}).get("block") or {}).get("id")

        # 결과보기(리더보드)
        if uter in ['결과보기','결과 보기','랭킹','랭킹보기','결과']:
            lb_text, mentions = _format_leaderboard_and_mentions(room, uid, limit=10)
            return jsonify({
                "version": "2.0",
                "template": {
                    "outputs": [
        ...
        if not st and uter in ['승부차기','승차']:
            ...
            return jsonify({
                ...
                "template": {
                    "outputs": [{
                        "simpleText": {"text": "📣 승부차기가 시작됩니다! 기회는 5번!\n🧍‍ vs 🧤\n“왼쪽, 가운데, 오른쪽” 중에 하나를 입력해주세요."}
                    }],
                    "quickReplies": [{"action":"message","label":o,"messageText":o}
                                     for o in ["왼쪽","가운데","오른쪽","왼쪽위","왼쪽아래","오른쪽위","오른쪽아래"]]
                }
            })
        ...
        if not dir_text or uter in ['승부차기','승차']:
            ...
            return jsonify({
                "version": "2.0",
                "template": {"outputs": [{
                    "simpleText": {"text": f"🧍‍ 키커 준비 완료! (진행 {n}/{st['max']}회)\n현재: {board}\n“왼쪽/가운데/오른쪽” 중 하나를 선택해 주세요."}
                }]},
                "extra": {"mentions": {"user1": {"type": "botUserKey", "id": uid}}}
            })

        # 판정
        success = (random.random() < _kick_prob(dir_text))
        shots, done = _record(room, uid, success)
        ...
        # 연속 멘트
        def _streak_tail_local(_shots, val):
            c = 0
            for s in reversed(_shots):
                if s is val: c += 1
                else: break
            return c
        g_streak = _streak_tail_local(shots, True)
        m_streak = _streak_tail_local(shots, False)

        if success:
            head = "골! "
            vibe = goal_streak_msg(g_streak) or _pick(GOAL_BASE)
            gk_line = _pick(["🧤 골키퍼가 움직이기도 전에 훅!","🧤 골키퍼가 반대편으로 뛰었네요!","🧤 완벽하게 속였습니다!"])
        else:
            head = "노골! "
            vibe = miss_streak_msg(m_streak) or _pick(MISS_BASE)
            gk_line = _pick(["🧤 골키퍼가 읽었어요!","🧤 손끝에 살짝 걸렸습니다!","🧤 코스가 들켰나 봐요!"])

        prefix = "{{#mentions.user1}}" + f" {head} {board} ({n}/5회)\n🧍‍ vs 🧤  |  현재 스코어 {total}골"
        reaction = f"\n{vibe}\n{gk_line}"

        if done:
            _career_add(room, uid, total, len(shots))  # ✅ 방 누적 저장
            badge = end_badge(total)
            summary = f"\n\n📣 게임 종료! {total}/5 성공! (성공률 {round(total/5*100)}%)\n{badge}\n"
            card = {
                "textCard": {
        ...
        return jsonify({
            "version": "2.0",
            "template": {"outputs": [{
                "simpleText": {"text": "문제가 발생했어요. '@피파봇 승부차기'로 다시 시작해 주세요."}
            }]}
        })
